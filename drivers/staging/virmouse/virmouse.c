...
//        printk ("virmouse_event: X:%d Y:%d %d %d\n", x, y, key,state);
        ...
        if (key>0) {
                if (key==1)
                        ...
                        input_report_key(virmouse_input_dev, BTN_MIDDLE, state);
                else if (key==3)
						...
						input_report_rel(virmouse_input_dev, REL_HWHEEL  , 1);
				else if (key==7)
						input_report_rel(virmouse_input_dev, REL_HWHEEL, -1);
        }
